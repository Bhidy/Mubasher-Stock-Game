...
// ============ IN-MEMORY STORAGE ============
// Persistent in Express process (better than serverless)
...
function calculateSentiment(tweets) {
    ...
    return { score, bullish: bull, bearish: bear, neutral: neut, total };
}
...
router.get('/', async (req, res) => {
    const { tab = 'fresh', refresh, market = 'SA' } = req.query;
    ...
    try {
        ...
        // Use Demo Data if Store is weak (Fallback for IP Blocks)
        ...
        const hotTickers = Object.entries(mentionCounts)
            .sort((a, b) => b[1] - a[1])
            .slice(0, 5)
        ...
        res.json({
            success: true,
            tab,
            tweets: filteredTweets.slice(0, 100),
            leaderboard,
            sentiment,
            hotTickers,
